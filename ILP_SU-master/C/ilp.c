...
ILP_make_vector(ILP_Object o1, ILP_Object o2){
    ...
    for(int i = 0; i < size; i++){
       tab->_content.asVector.vector[i] = o2;
    }
    return tab;
}
...
ILP_vector_length(ILP_Object o){
	if(!ILP_isVector(o)){
		printf("o n'est pas un vector");
		return NULL;
	}
	int size = o->_content.asVector.size;
	return ILP_make_integer(size);
}

ILP_Object
ILP_vector_get(ILP_Object o1, ILP_Object o2){
	if(!ILP_isVector(o1)){
		printf("o1 n'est pas un vector\n");
		return NULL;
	}
	if (!ILP_isInteger(o2)){
		printf("o2 n'est pas un integer\n");
		return NULL;
	}
	int index = o2->_content.asInteger;
	if (o1->_content.asVector.size <= index){
		printf("index est trop grand\n");
		return NULL;
	}
	return o1->_content.asVector.vector[index];
}
